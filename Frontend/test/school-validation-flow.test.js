...
describe('PR 5 school validation flow frontend', () => {
  ...
  it('registers school SDD routes and routes shared pages by role', async () => {
    ...
    assert.match(layoutSource, /label: 'Konfirmasi & Validasi'/)
    assert.match(layoutSource, /path: '\/validasi'/)
    assert.doesNotMatch(layoutSource, /path: '\/validasi\?mode=konfirmasi'/)
    assert.doesNotMatch(layoutSource, /path: '\/validasi\?mode=validasi'/)
